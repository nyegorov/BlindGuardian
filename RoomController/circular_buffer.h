...
public:
	...
	explicit circular_buffer(size_type capacity = default_capacity)
		: array_(alloc_.allocate(capacity)), array_size_(capacity), alloc_size_(capacity),
		head_(1), tail_(0), contents_size_(0)
	{
	}
	circular_buffer(const circular_buffer &other)
		: array_(alloc_.allocate(other.alloc_size_)),
		...
		contents_size_(other.contents_size_)
	{
		...
		catch (...)
		{
			...
			alloc_.deallocate(array_, alloc_size_);
			throw;
		}
	}
	template <class InputIterator>
	circular_buffer(InputIterator from, InputIterator to)
		: array_(alloc_.allocate(1)), array_size_(1), alloc_size_(1),
		head_(1), tail_(0), contents_size_(0)
	{
		circular_buffer tmp;
...
private:
	...
	void assign_into_reserving(f_iter from, f_iter to)
	{
		...
		while (from != to)
		{
			if (contents_size_ == alloc_size_)
			{
				reserve(static_cast<size_type>(alloc_size_ * 1.5));
			}
			push_back(*from);
			++from;
		}
